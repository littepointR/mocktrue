...
- [Architecture Decisions](development/architecture-decisions.md) — 当前 Wails 架构、MCP-first、Wails v3 alpha guardrails，以及受控 Qt/QML + Rust 迁移 ADR。
- [Qt/QML + Rust Migration Playbook](development/qt-rust-migration-playbook.md) — 若接受新 ADR 后执行 Qt/QML + Rust 迁移，用于功能盘点、团队/agent 分工、门禁和最终验收。
- [Migration Feature Matrix](development/migration-feature-matrix.csv) — Qt/QML + Rust 迁移的功能 ID、旧入口、新实现、测试和验收状态台账。
- [Migration Contracts](development/migration-contracts.md) — 第一波迁移开发前必须稳定的 Rust / Qt / QML 契约。
- [QML Design Brief](development/qml-design-brief.md) — `opus-qml-design-lead` 的前端信息架构、状态模型和 feature ID 映射任务书。
- [QML First-Wave Design Handoff](development/qml-first-wave-design.md) — 第一波 QML 页面清单、状态模型、导航模型和控件到 feature ID 映射。
- [Qt/Rust Skeleton Startup Package](development/qt-rust-skeleton-startup.md) — 进入新栈 skeleton 开发前的目录、agent 分工、命令、退出门槛和 stop 条件。
- [Migration Gap Report](development/migration-gap-report.md) — 当前盘点覆盖、环境阻塞、剩余缺口和下一步动作。
- [Migration Readiness Checklist](development/migration-readiness.md) — 从准备阶段进入 skeleton/功能开发前的硬门槛。
